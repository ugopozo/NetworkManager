                                         ...
                                         GError **error)
{
	...
	if (!nm_supplicant_config_add_option (self, "macsec_policy", "1", -1, FALSE, error))
		return FALSE;

	value = nm_setting_macsec_get_encrypt (setting) ? "0" : "1";
	if (!nm_supplicant_config_add_option (self, "macsec_integ_only", value, -1, FALSE, error))
		return FALSE;

	port = nm_setting_macsec_get_port (setting);
	if (port > 0 && port < 65534) {
		snprintf (buf, sizeof (buf), "%d", port);
		if (!nm_supplicant_config_add_option (self, "macsec_port", buf, -1, FALSE, error))
			return FALSE;
	}

	if (nm_setting_macsec_get_mode (setting) == NM_SETTING_MACSEC_MODE_PSK) {

		if (!nm_supplicant_config_add_option (self, "key_mgmt", "NONE", -1, FALSE, error))
			return FALSE;
